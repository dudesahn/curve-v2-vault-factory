...
def test_vault_deployment(
    ...
    curve_global.setConvexStratImplementation(convex_template, sender=gov)
    curve_global.setConvexFraxStratImplementation(frax_template, sender=gov)
    curve_global.setCurveStratImplementation(curve_template, sender=gov)
    ...
    curve_global.setKeepFXS(69, gov, sender=gov)
    print(
    ...
    vault.acceptGovernance(sender=gov)
    ...
    if frax_strat != ZERO_ADDRESS:
        ...
        vault.updateStrategyDebtRatio(frax_strategy, 3000, sender=gov)
        ...
        vault.updateStrategyDebtRatio(curve_strategy, 0, sender=gov)
        vault.updateStrategyDebtRatio(convex_strategy, 0, sender=gov)
        vault.updateStrategyDebtRatio(curve_strategy, 5000, sender=gov)
        vault.updateStrategyDebtRatio(convex_strategy, 5000, sender=gov)
...
def test_curve_global_setters_and_views(
    ...
    curve_global.setKeepCRV(69, gov, sender=gov)
    ...
    curve_global.setGuardian(gov, sender=gov)
    assert curve_global.guardian() == gov.address
    curve_global.setConvexPoolManager(gov, sender=gov)
    assert curve_global.convexPoolManager() == gov.address
    curve_global.setBooster(gov, sender=gov)
    assert curve_global.booster() == gov.address
    curve_global.setGovernance(gov, sender=gov)
    assert curve_global.governance() == gov.address
    curve_global.setManagement(gov, sender=gov)
    assert curve_global.management() == gov.address
    curve_global.setGuardian(gov, sender=gov)
    assert curve_global.guardian() == gov.address
    curve_global.setTreasury(gov, sender=gov)
    assert curve_global.treasury() == gov.address
    curve_global.setTradeFactory(gov, sender=gov)
    assert curve_global.tradeFactory() == gov.address
    curve_global.setBaseFeeOracle(gov, sender=gov)
    assert curve_global.baseFeeOracle() == gov.address
    curve_global.setConvexStratImplementation(gov, sender=gov)
    assert curve_global.convexStratImplementation() == gov.address
    curve_global.setCurveStratImplementation(gov, sender=gov)
    assert curve_global.curveStratImplementation() == gov.address
    curve_global.setManagementFee(69, sender=gov)
    assert curve_global.managementFee() == 69
    curve_global.setPerformanceFee(69, sender=gov)
    assert curve_global.performanceFee() == 69
    curve_global.setOwner(whale, sender=gov)
    curve_global.acceptOwner(sender=whale)
