import pytest
from ape import Contract, project, accounts, chain
import requests
import os
from utils import ZERO_ADDRESS, increase_time
...
def whale(accounts, amount, token, whale_accounts, token_string):
    ...
    whale = accounts[whale_accounts[token_string]]
...
def profit_whale(accounts, profit_amount, token, profit_whale_accounts, token_string):
    ...
    profit_whale = accounts[profit_whale_accounts[token_string]]
...
@pytest.fixture(scope="session")
def contract_name(which_strategy):
    if which_strategy == 0:
        contract_name = project.StrategyConvexFactoryClonable
    elif which_strategy == 1:
        contract_name = project.StrategyCurveBoostedFactoryClonable
    elif which_strategy == 2:
        contract_name = project.StrategyPrismaConvexFactoryClonable
    elif which_strategy == 3:
        contract_name = project.StrategyConvexFxnFactoryClonable
    else:
        contract_name = project.StrategyConvexFraxFactoryClonable
    yield contract_name
...
if chain_used == 1:  # mainnet
    ...
    def health_check():
        yield Contract(
            "0xddcea799ff1699e98edf118e0629a974df7df012", abi="abis/IHealthCheck.json"
        )

    @pytest.fixture(scope="session")
    def base_fee_oracle():
        yield Contract(
            "0xfeCA6895DcF50d6350ad0b5A8232CF657C316dA7", abi="abis/IBaseFeeOracle.json"
        )

    # set all of the following to SMS, just simpler
    @pytest.fixture(scope="session")
    def management():
        yield accounts["0x16388463d60FFE0661Cf7F1f31a7D658aC790ff7"]
    ...
    def release_registry():
        yield Contract("0x7Cb5ABEb0de8f6f46a27329B9eF54CE10E47F1e2")


@pytest.fixture(scope="module")
def vault(gov, rewards, guardian, management, token, vault_address, release_registry):
    if vault_address == ZERO_ADDRESS:
        tx = release_registry.newVault(
            token, gov, guardian, rewards, "", "", 0, sender=gov
        )
        # vault_address = tx.return_value
        for log in release_registry.NewClone.from_receipt(tx):
            vault_address = log.vault
        vault = Contract(vault_address, abi="abis/IVaultFactory045.json")
...
@pytest.fixture(scope="session")
def other_strategy():
    yield Contract("0x2f00C2B4B5BBa25C72106595551073efc6Fa18F4")
...
def strategy(
    ...
    if which_strategy == 0:  # convex
        ...
        strategy.setHarvestTriggerParams(
            90_000 * 10**6, 150_000 * 10**6, strategy.checkEarmark(), sender=gov
        )
    elif which_strategy == 1:  # Curve
        vault.addStrategy(strategy, 10_000, 0, 2**256 - 1, 0, sender=gov)
        ...
        if has_rewards:
            ...
            strategy.updateRewards([rewards_token], sender=gov)
            new_proxy.approveRewardToken(strategy.rewardsTokens(0), sender=gov)

        # approve our new strategy on the proxy (if we want to test an existing want, a bit more work is needed)
        if new_proxy.strategies(strategy.gauge()) != ZERO_ADDRESS:
            # revoke strategy on gauge
            new_proxy.revokeStrategy(strategy.gauge(), sender=gov)
        ...
        new_proxy.approveStrategy(strategy.gauge(), strategy, sender=gov)
        ...
        strategy.setClaimParams(False, True, sender=gov)
    elif which_strategy == 3:  # FXN Convex
        vault.addStrategy(strategy, 10_000, 0, 2**256 - 1, 0, sender=gov)
    ...
    base_fee_oracle.setBaseFeeProvider(ZERO_ADDRESS, sender=management)
    base_fee_oracle.setManualBaseFeeBool(True, sender=management)
...
def rewards_whale(accounts):
    ...
    yield accounts["0x28C6c06298d514Db089934071355E5743bf21d60"]
...
@pytest.fixture(scope="session")
def fxn_whale():
    yield accounts["0x26B2ec4E02ebe2F54583af25b647b1D619e67BbF"]
...
def gauge(pid, booster):
    ...
    yield Contract(gauge, abi="abis/ICurveGaugeV6.json")
