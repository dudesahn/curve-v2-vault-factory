...
def test_change_debt(
    ...
    # yswaps will not have taken this first batch of profit yet, but the assets will be sitting in the strategy
    if use_yswaps:
        assert vault.creditAvailable(strategy) == 0
...
def test_change_debt_with_profit(
    ...
    reduced = int(
        starting_debt_ratio / 2
    )  # we have to wrap this as an integer for ape for some reason
    vault.updateStrategyDebtRatio(strategy, reduced, sender=gov)
    ...
    donation = int(amount / 4)  # again, ape needs to be told it's an integer
