...
def test_emergency_exit(
    ...
    # again, harvests in emergency exit don't enter prepareReturn, so we need to claim yPRISMA manually
    if which_strategy == 2:
...
def test_emergency_exit_with_loss(
    ...
    if which_strategy == 0:
        ...
        rewards_contract.withdrawAllAndUnwrap(True, sender=strategy)
        ...
        gauge.transfer(gov, to_send, sender=voter)
        assert strategy.estimatedTotalAssets() == 0
        ...
        kek = stake[0]
        user_vault.withdrawLocked(kek, sender=strategy)
        staking_token.transfer(gov, staking_token.balanceOf(strategy), sender=strategy)
        assert strategy.estimatedTotalAssets() == 0
    ...
    # prisma strategy doesn't earn anything here
    if which_strategy == 2:
        no_profit = True
...
def test_emergency_exit_with_no_loss(
    ...
    if which_strategy == 0:
        ...
        assert strategy.estimatedTotalAssets() == 0
        # gov withdraws
        gauge.withdraw(to_send, sender=gov)
    elif which_strategy == 2:
        to_send = prisma_receiver.balanceOf(strategy)
        prisma_receiver.withdraw(gov, to_send, sender=strategy)
    elif which_strategy == 3:
    ...
    if which_strategy in [0, 1]:
        ...
        fxn_gauge.transfer(user_vault, to_send, sender=gov)
        assert strategy.estimatedTotalAssets() > 0
    elif which_strategy == 4:
        # gov unwraps and sends it back, glad someone was watching!
        to_unwrap = staking_token.balanceOf(gov)
        staking_token.withdrawAndUnwrap(to_unwrap, sender=gov)
        token.transfer(strategy, to_unwrap, sender=gov)
        assert strategy.estimatedTotalAssets() > 0
...
def test_emergency_withdraw_method_0(
    ...
    strategy.withdrawToConvexDepositTokens(sender=gov)
    ...
    strategy.sweep(cvx_deposit, sender=gov)
...
def test_emergency_withdraw_method_1(
    ...
    strategy.setClaimRewards(False, sender=gov)
    strategy.withdrawToConvexDepositTokens(sender=gov)
